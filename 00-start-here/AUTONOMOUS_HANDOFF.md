...
- **M1: integration committed `0ed3d6d`** (full 13-route catalog, clock pricing, Anthropic wire, thinking dialects, stateless MCP client, `nh chat` REPL). A background Fable-5 workflow (`build-m1-nosis-harness`) was still finishing **Review → Harden → Ship** at handoff — so expect 1–2 more M1 commits (hardening fixes + docs) to have landed. 11 test binaries were green.
- `CONTRACTS_M1.md` exists at repo root (pattern to copy for M2+).

## FIRST ACTIONS on resume

1. `git log --oneline | head -8` — confirm M1 fully landed (look for an "M1: hardening" and/or "docs: M1" commit after `0ed3d6d`). If the M1 workflow left the tree dirty or red, finish/verify M1 first.
2. `cargo test --workspace` + `cargo clippy --workspace --all-targets -- -D warnings` — must be green before M2.
